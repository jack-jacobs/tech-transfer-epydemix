...
model_vax <- EpiModel(name = "SIR with Vaccination", compartments = c("S", "SV", "I", "R"))

# S + I → I (unvaccinated infection)
params_SI <- builtins$tuple(list("beta", "I"))
model_vax$add_transition("S", "I", params = params_SI, kind = "mediated")

# SV + I → I (vaccinated infection, reduced by VE)
params_SVI <- builtins$tuple(list("(1 - VE) * beta", "I"))
model_vax$add_transition("SV", "I", params = params_SVI, kind = "mediated")
...
```{r viz-vax-compartments}
# Plot compartment evolution
df_vax_quantiles <- results_vax$get_quantiles_compartments()

plot_quantiles(
  df_vax_quantiles,
  columns = c("S_total", "SV_total", "I_total", "R_total"),
  title = "SIR Model with Vaccination Campaign"
)
plt$show()
```

```{r viz-vax-age-groups}
# Vaccinated individuals by age group
plot_quantiles(
  df_vax_quantiles,
  columns = c("SV_0-4", "SV_5-19", "SV_20-49", "SV_50-64", "SV_65+"),
  title = "Vaccinated Susceptibles by Age Group",
